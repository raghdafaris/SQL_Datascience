...
SELECT 
  ...
  test_events.user_id,
  MAX(CASE WHEN orders_tbl.created_at > test_events.event_time THEN 1 ELSE 0 END) AS orders_after_assignment_binary
FROM
(
  SELECT 
    user_id,
    event_time,
    MAX(CASE WHEN parameter_name = 'test_id' THEN CAST(parameter_value AS INT) END) AS test_id,
    MAX(CASE WHEN parameter_name = 'test_assignment' THEN CAST(parameter_value AS INT) END) AS test_assignment
  FROM 
    dsv1069.events
  WHERE 
...
LEFT JOIN 
  dsv1069.orders orders_tbl
ON 
  orders_tbl.user_id = test_events.user_id
GROUP BY 
  test_events.test_id,
  test_events.test_assignment,
  test_events.user_id;
